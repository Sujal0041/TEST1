...
const Transactions = () => {
  ...
  const renderTransactionItem = ({item}) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.transactionItem, itemStyle]}
        onPress={() =>
          navigation.navigate('DeleteTransaction', {
            transaction: item,
          })
        }>
        <FontAwesomeIcon
        ...
        <View style={styles.transactionDetails}>
          ...
          <View>
            <Text style={{color: amountColor, fontSize: 16}}>
              {wallets.find(wallet => wallet.id === item.wallet)?.currency}{' '}
              {amountText}
            </Text>
