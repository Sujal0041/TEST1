...
const EditTransaction = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const fetchedCategories = await getAllCategories(userToken);
        const fetchedWallets = await getAllWallets(userToken);

        const category = fetchedCategories.find(
          cat => cat.id === transaction?.category,
        );
        const wallet = fetchedWallets.find(
          wal => wal.id === transaction?.wallet,
        );

        setCategories(fetchedCategories);
        setWallets(fetchedWallets);

        setCategoryName(category ? category.category_name : 'Unknown');
        setWalletName(wallet ? wallet.name : 'Unknown');
      } catch (error) {
